...
/**
 * The full conversational surface — history load + live-update poll + the
 * message thread + the input dock + the inline pending-tasks widget. Rendered
 * in two shells:
 *   - `variant="page"`  → the standalone /chat route (a centered <main>, dock
 *                          sticky to the viewport bottom).
 *   - `variant="island"`→ inside the app-wide Chat Island bottom sheet (fills
 *                          its parent, dock is a normal bottom row).
 * `active` gates the 13s live-update poll: the island passes `false` while
 * collapsed so a background presence never burns the Neon compute budget.
 */
export function ChatPanel({
  ...
  useEffect(() => {
    // `active` is false while the island is collapsed — a background chat
    // presence must never poll (Neon compute budget). The page variant is
    // always active. When the island expands, `active` flips true and this
    // effect re-subscribes, catching up on any server-side messages.
    if (!historyLoaded || !active) return;
    let cancelled = false;
    const poll = async () => {
      if (cancelled) return;
  ...
  }, [historyLoaded, status, setMessages, active]);
  ...
  // Dock placement is variant-aware: on the /chat page it sticks to the
  // viewport bottom; inside the island sheet it's the last row of the flex
  // column (the sheet itself owns the bottom edge).
  const dockClass = isPage
    ? "sticky bottom-0 z-20 bg-[var(--c-void)] -mx-3 sm:-mx-4 px-3 sm:px-4 pb-[max(0.5rem,env(safe-area-inset-bottom))]"
    : "shrink-0 bg-[var(--c-void)] -mx-3 sm:-mx-4 px-3 sm:px-4 pt-1 pb-[max(0.5rem,env(safe-area-inset-bottom))]";
  ...
  return isPage ? (
    ...
    // Island shell — fills the bottom-sheet body: the thread scrolls, the
    // dock is the last row of this column (the sheet owns the bottom edge).
    <div className="flex flex-col h-full min-h-0 px-3 sm:px-4 pt-2">{content}</div>
  );
}
